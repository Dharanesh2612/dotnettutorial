...
namespace APIConsume.Controllers
{
    public class HomeController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            {
                using (var response = await httpClient.GetAsync("http://localhost:5137/api/Reservation"))
                {
        ...
        public async Task<IActionResult> GetReservation(int id)
        {
            ...
            using (var httpClient = new HttpClient())
            {
                using (var response = await httpClient.GetAsync("http://localhost:5137/api/Reservation/" + id))
                {
       ...
       public async Task<IActionResult> AddReservation(Reservation reservation)
        {
            ...
            using (var httpClient = new HttpClient())
            {
                ...
                using (
                    var response = await 
                    httpClient.PostAsync("http://localhost:5137/api/Reservation", content))
                {
                    string apiResponse = await response.Content.ReadAsStringAsync();
        ...
        public async Task<IActionResult> UpdateReservation(int id)
        {
            ...
            {
                using (var response =
                    await httpClient.GetAsync("http://localhost:5137/api/Reservation/" + id))
                {
                    if (response.StatusCode == System.Net.HttpStatusCode.OK)
        ...
        public async Task<IActionResult> UpdateReservation(Reservation reservation)
        {
            ...
            using (var httpClient = new HttpClient())
            {
                ...
                using (var response = await httpClient.PutAsync("http://localhost:5137/api/Reservation/", content))
                {
        ...
        public async Task<IActionResult> DeleteReservation(int Reservation)
        {
            using (var httpClient = new HttpClient())
            {
                using (var response = await httpClient.DeleteAsync("http://localhost:5137/api/Reservation/" + Reservation))
                {
                    string apiResponse = await response.Content.ReadAsStringAsync();
                }
            }
